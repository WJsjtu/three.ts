...
class Mesh extends Object3D {
    ...
    public updateMorphTargets(): this {
        ...
        if (geometry instanceof BufferGeometry) {
            ...
            const keys: Array<string> = Object.keys(morphAttributes);
            if (keys.length > 0) {
                const morphAttribute: Array<BufferAttribute> = morphAttributes[keys[0]];
    ...
    public static uvIntersection(point: Vector3, triangle: Triangle, uv1: Vector2, uv2: Vector2, uv3: Vector2): Vector2 {
    ...
    public raycast(raycaster: Raycaster, intersects: Array<IIntersection> = []): Array<IIntersection> {
        ...
        if (geometry instanceof BufferGeometry) {
            ...
            for (let f: number = 0, fl: number = faces.length; f < fl; f++) {
                ...
                const faceMaterial: MeshBasicMaterial = Array.isArray(material) ? material[face.materialIndex] : material;
                ...
                if (intersection) {
                    if (uvs && uvs[f]) {
                        const uvs_f: Array<Vector2> = uvs[f];
    ...
    public clone (): Mesh {
		return (new (this.constructor as (geometry: BufferGeometry | Geometry, material: MeshBasicMaterial | Array<MeshBasicMaterial>) => void)( this.geometry, this.material)).copy( this );
	}
}